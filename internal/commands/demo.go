...
func RunDemo(app *App) error {
	...
	// Convert WorkItems into jira.Issue records so the MockClient can
	// look them up for transitions, comments, assignments, edits, etc.
	clientIssues := make([]jira.Issue, 0, len(items))
	for _, item := range items {
		iss := jira.Issue{
			...
			Fields: jira.IssueFields{
				...
				Priority:  jira.Priority{Name: item.StringField("priority")},
				IssueType: jira.IssueType{Name: item.Type},
				Created:   item.StringField("created"),
				Updated:   item.StringField("updated"),
			},
		}
		if item.Description != nil {
			adfMap := jira.RenderADFValue(item.Description)
			if raw, err := json.Marshal(adfMap); err == nil {
				iss.Fields.Description = raw
			}
		}
		if item.ParentID != "" {
			iss.Fields.Parent = &jira.ParentRef{Key: item.ParentID}
		}
		clientIssues = append(clientIssues, iss)
	}

	// Register the demo workspace in the config so Upsert can resolve it.
	if app.Config.Workspaces == nil {
		app.Config.Workspaces = make(map[string]*core.Workspace)
	}
	app.Config.Workspaces[ws.Slug] = ws
	app.Config.DefaultWorkspace = ws.Slug

	jiraCfg := ws.ProviderConfig.(*jira.Config)

	// Install a MockClient so TUI actions work against in-memory data.
	mock := jira.NewMockClient(clientIssues, ws.Statuses, jiraCfg.ProjectKey)
...
func demoWorkspace() *core.Workspace {
