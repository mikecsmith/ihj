...
func Workspace() *core.Workspace {
	...
	return &core.Workspace{
		...
		Filters:        map[string]string{"active": "", "my": "assignee:me", "custom": "custom"},
	}
}
...
func Issues() []*core.WorkItem {
	...
	return []*core.WorkItem{
		...
		// ── Scrum board items (visible via "custom" filter) ────────────
		{
			...
			Fields: map[string]any{
				...
				"created":  d(30),
				"updated":  d(1),
				"labels":   []string{"platform", "automation"},
				"sprint":   "Sprint 42",
				"team":     "Platform",
			},
			Description: md("## Goal\n\nAutomate sprint planning ceremonies and velocity tracking.\n\n## Key Results\n\n- Velocity auto-calculated from completed story points\n- Burndown chart updates in real time\n- Sprint scope changes tracked with audit log"),
		},
		{
			...
			Description: md("Calculate team velocity from the last 3 sprints.\n\n## Acceptance Criteria\n\n- Given completed stories, when the sprint ends, then velocity is averaged over last 3 sprints\n- Given no historical data, when the first sprint completes, then velocity equals that sprint's total"),
		},
		{
			ID: "SCRUM-3", Summary: "Build burndown chart widget",
			Type: "Story", Status: "To Do", ParentID: "SCRUM-1",
			Fields: map[string]any{
				"priority":     "Medium",
				"assignee":     "Jordan Park",
				"reporter":     "Sarah Chen",
			...
			Description: md("Render a burndown chart showing ideal vs actual remaining story points per day."),
		},
		{
			ID: "SCRUM-4", Summary: "Configure sprint board columns",
			Type: "Task", Status: "Done", ParentID: "SCRUM-1",
			Fields: map[string]any{
				"priority":     "Low",
				"assignee":     "Priya Patel",
				"reporter":     "Mike Smith",
		...
		{
			ID: "SCRUM-5", Summary: "Velocity miscalculates after mid-sprint scope change",
			Type: "Bug", Status: "To Do",
			Fields: map[string]any{
				"priority":     "High",
				"assignee":     "Alex Rivera",
			...
			Description: md("## Steps to Reproduce\n\n1. Start sprint with 30 SP committed\n2. Add a 5 SP story mid-sprint\n3. Complete all original stories\n\n## Expected\n\nVelocity reflects 30 SP completed (original commitment)\n\n## Actual\n\nVelocity shows 30/35 = 86% instead of 30 SP"),
		},
		{
			ID: "SCRUM-6", Summary: "Add story point field to sub-tasks",
			Type: "Sub-task", Status: "In Progress", ParentID: "SCRUM-2",
			Fields: map[string]any{
				"priority":     "Medium",
				"assignee":     "Alex Rivera",
				"reporter":     "Sarah Chen",
